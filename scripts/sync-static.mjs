...
const app = path.join(root, "frontoffice", "dist");
const pub = path.join(root, "public");
const adminOut = path.join(pub, "admin");
const appOut = path.join(pub, "app");
...
if (!existsSync(path.join(app, "index.html"))) {
  throw new Error("Frontoffice build missing. Run npm --prefix frontoffice run build first.");
}
...
if (existsSync(appOut)) rmSync(appOut, { recursive: true, force: true });
cpSync(app, appOut, { recursive: true });

console.log("Synced shop → /, backoffice → /admin, frontoffice → /app");
