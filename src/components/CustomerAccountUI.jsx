...
export function AccountFormActions({ primaryLabel, primaryBusy, secondary, secondaryLabel }) {
  return (
    <>
      <button className="btn btn-green btn-block account-form-submit" type="submit" disabled={primaryBusy}>
  ...
  return (
    <form className="order-review-card" onSubmit={submit}>
      ...
      <div className="star-row star-row-lg">
        {[1, 2, 3, 4, 5].map((n) => (
          <button key={n} type="button" className={"star-pick" + (rating >= n ? " on" : "")} onClick={() => setRating(n)} aria-label={n + " stars"}>
            ★
          </button>
        ))}
      </div>
      ...
      {msg ? <p className="customer-auth-msg err">{msg}</p> : null}
      <div className="order-review-actions">
        ...
        <button className="btn btn-green btn-sm" type="submit" disabled={busy}>
